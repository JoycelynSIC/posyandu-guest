        ...
        <form method="GET" action="{{ route('warga.index') }}" class="wow fadeInUp mb-4 mt-3" data-wow-delay="0.25s">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="search" class="form-control" value="{{ request('search') }}" placeholder="Cari nama, pekerjaan, email, no KTP, atau telp...">
                </div>
                ...
                <div class="col-12 d-flex gap-2 mt-2">
                    ...
                    @if(request('search') || request('jenis_kelamin') || request('agama'))
                        <a href="{{ route('warga.index') }}" class="btn btn-outline-primary shadow-sm rounded-pill flex-fill py-2 px-3 d-flex align-items-center justify-content-center">Clear</a>
                    @endif
                </div>
            </div>
        </form>
     ...
     style="width:80px; height:80px; object-fit:cover;">
                        ...
                        <div class="card-footer bg-transparent border-0 text-center pb-3">
                            <div class="d-flex justify-content-center align-items-center gap-2">
                                <a href="{{ route('warga.edit', $data->warga_id) }}" class="btn btn-outline-primary btn-sm d-flex align-items-center justify-content-center gap-1 shadow-sm" style="min-width: 90px; height: 35px;">
                                    <i class="fa fa-pen"></i> Edit
                                </a>
                                <form action="{{ route('warga.destroy', $data->warga_id) }}" method="POST" class="d-flex m-0 p-0">
                                    @csrf
                                    @method('DELETE')
