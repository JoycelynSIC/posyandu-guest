...
<div class="py-3 bg-light d-flex justify-content-center">
    <div class="card shadow-sm border-0 rounded-4 wow fadeInUp" data-wow-delay="0.1s"
        ...
        <div class="card-body p-3">
            ...
            <!-- Form -->
            <form action="{{ route('kader.update', $kader->kader_id) }}" method="POST" class="wow fadeInUp" data-wow-delay="0.3s" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="mb-2">
                    ...
                    <select name="posyandu_id" class="form-select form-select-sm @error('posyandu_id') is-invalid @enderror" required>
                        ...
                        @foreach ($posyandu as $pos)
                            <option value="{{ $pos->posyandu_id }}" {{ old('posyandu_id', $kader->posyandu_id)==$pos->posyandu_id?'selected':'' }}>
                ...
                <div class="mb-2">
                    ...
                    <select name="warga_id" class="form-select form-select-sm @error('warga_id') is-invalid @enderror" required>
                        ...
                        @foreach ($warga as $w)
                            <option value="{{ $w->warga_id }}" {{ old('warga_id', $kader->warga_id)==$w->warga_id?'selected':'' }}>
                                {{ $w->nama }} - {{ $w->no_ktp }}
                            </option>
                        @endforeach
                    </select>
