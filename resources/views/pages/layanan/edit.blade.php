    ...
    <div class="py-3 bg-light d-flex justify-content-center">
        <div class="card shadow-sm border-0 rounded-4 wow fadeInUp" data-wow-delay="0.1s"
            ...
            <div class="card-body p-3">
                ...
                <!-- Form -->
                <form action="{{ route('layanan.update', $layanan->layanan_id) }}" method="POST" class="wow fadeInUp">
                    @csrf
                    @method('PUT')

                    <input type="hidden" name="jadwal_id" value="{{ $layanan->jadwal_id }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="warga_id"
                            ...
                            @foreach($warga as $w)
                                <option value="{{ $w->warga_id }}" {{ ($layanan->warga_id == $w->warga_id) ? 'selected' : '' }}>
                                    {{ $w->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.1" name="berat"
                                ...
                                value="{{ old('berat', $layanan->berat) }}" placeholder="Contoh: 12.5" required>
                            @error('berat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.1" name="tinggi"
                                ...
                                value="{{ old('tinggi', $layanan->tinggi) }}" placeholder="Contoh: 85.5" required>
                            @error('tinggi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="vitamin" class="form-control @error('vitamin') is-invalid @enderror"
                            value="{{ old('vitamin', $layanan->vitamin) }}" placeholder="Contoh: Vitamin A, C">
                        @error('vitamin')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="konseling" class="form-select @error('konseling') is-invalid @enderror" required>
                            <option value="1" {{ $layanan->konseling == '1' ? 'selected' : '' }}>Ya</option>
                            <option value="0" {{ $layanan->konseling == '0' ? 'selected' : '' }}>Tidak</option>
                        </select>
                        @error('konseling')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex justify-content-between mt-3 wow fadeInUp" data-wow-delay="0.4s">
                        <a href="{{ url()->previous() }}" class="btn btn-primary rounded-pill px-3 py-1">
